...
if (isset($_POST['nama_lengkap'])) {
    ...
    if (isset($_FILES['foto'])) {
        ...
        if (move_uploaded_file($foto_tmp, $path)) {
            ...
            $query = mysqli_query($koneksi, "UPDATE user SET nama_lengkap='$nama',username='$username',foto='$fotobaru' WHERE id_user='$id'");
            $session = mysqli_query($koneksi, "SELECT * FROM user WHERE id_user='$id'");
            if ($query) {
                $_SESSION['user'] = mysqli_fetch_array($session);
                echo '<script>alert("Profil Berhasil di Update");localhost.href="?page=profil";</script>';
            }
        }
    }
            ...
            <div class="card-body h-100">
                <form action="" method="post" enctype="multipart/form-data">
                    <div class="mb-3">
                        ...
                        <div class="col-12">
                            <input type="text" name="nama_lengkap" class="form-control" value="<?php echo $_SESSION['user']['nama_lengkap'] ?>">
                        </div>
                    </div>
                    <div class="mb-3">
